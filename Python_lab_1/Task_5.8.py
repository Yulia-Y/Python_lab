...
if nim>0:
    ...
    num=1
    get=st1
print("Компьютер забрал " + str(dif) + " камней из " + str(num) + " кучи, теперь там " + str(get) + " камней. Твой ход")
while st1+st2+st3>0:
    step=2
    num=0
    ...
    while con:
        ...
        if dif>0 and dif<=get:
            if num==1:
                ...
                st3-=dif
                get=st3
            print("Ты забрал " + str(dif) + " камней из " + str(num) + " кучи, теперь там " + str(get) + " камней. Ход компьютера")
            con=False
    if st1+st2+st3>0:
        step=1
        nim=st1^st2^st3
